...
The Aviatrix Controller has pushed a `Default Action Rule` permitting all traffic; you should observe the impact right away. Please note that for this specific ruleset (i.e. _Policy Rules Policy List (System))_, you cannot add new rules. You may only modify the existing "Default Action Rule."
...
Let’s move to the `V1 Policy List`, where you can create new rules that will be placed above the Default Action Rule.
...
```{important}
Applying the `All-Web` WebGroup to the Greenfield rule enables FQDN logging, specifically within the Egress section. Please note that because this is the top-most rule, it holds the highest priority. Since the WebGroup restricts traffic solely to HTTP/HTTPS, all other East-West traffic will be denied.
```

This rule acts as a `Discovery rule`, allowing you to identify all traffic generated by the internal database virtual machines and review it within the Monitor section. Once you enforce the policy in the data plane and click **Commit**, the default WebGroup will effectively block non-web traffic while ensuring full visibility into your database communications.
...
In the next tasks, you will whitelist the permitted domains. 

Anything else—in this case, blocking _`www.malware.net`_ while allowing all others—will be denied and dropped, triggering the `ExplicitDenyAll` rule.
...
```{tip}
If you are not using a corporate laptop, you may access the DB virtual machines using your preferred SSH client. First, SSH into the **Frontend** instance, which serves as your _bastion host_. Once logged into either DB virtual machine, attempt to generate traffic toward both the allowed and prohibited domains. Please note that the two policies permitting **TCP/22** traffic between the Frontend and BU1-DB, as well as the Frontend and BU2-DB, are indeed enforced within the **_Cloud Native Security Fabric_**.
```
